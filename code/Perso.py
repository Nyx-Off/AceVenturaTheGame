...
class Perso(GameObject):
    def __init__(self, image_paths, width, height):
        ...
        self.list_over = ["code/assets/AceVenturaCharacter_3.png", "code/assets/AceVenturaCharacter_dead.png",
                          "code/assets/AceVenturaCharacter_dead.png"]
        self.index_over = 0
        self.over = False
    ...
    def update(self):
        ...
        if current_time - self.last_frame_change >= self.frame_duration and self.game_over:
            self.image = py.image.load(self.image_paths[self.current_frame_index]).convert_alpha()
            self.image = py.transform.scale(self.image, (self.character_width, self.character_height))
